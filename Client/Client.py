...
class Client:
    ...
    def receiving_video(self,ip):
        try:
            ...
            # Previously fell straight through to self.connection.read(4)
            # below with self.connection never set, crashing with
            # AttributeError instead of failing cleanly like the command
            # socket's connect handler does.
